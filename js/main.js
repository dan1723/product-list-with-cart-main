...
function toggleButton() {
    ...
    // Use a delegação de eventos para lidar com cliques no botão .cart-button.
    gridWrap.addEventListener("click", (e) => {
        // Look for the closest element with the class "cart-button"
        const cartButton = e.target.closest(".cart-button");

        if (cartButton) {
            // Use nextElementSibling para obter o símbolo de mais ou menos do carrinho imediatamente após o botão.
            const cartPlusMinus = cartButton.nextElementSibling;


            if (cartPlusMinus) {
                if (cartButton.classList.contains("active")) {
                    // Remova "ativo" do botão e adicione-o ao carrinho (com mais ou menos).
...
function addToCart(gridItem) {
    ...
    // Adicionar funcionalidade para remover o item do carrinho.
